# DETECT using Snakemake 9

Tested with the latest version of Snakemake available 9.13.4 (11-04-2025)

## Quick Start

### 1. Build the Conda Environment

```shell
module load mamba/latest
mamba env create --file DETECT/DETECT.yml
```

### 2. Create Configuration Files

Use the interactive configuration script to generate `config.sh`, `run.sh`, and `sbatch_run.sh`:

```shell
bash scripts/create_config.sh
```

This script will:
- Prompt you for all required parameters (reference genome, VCF files, coverages, etc.)
- Prompt for SLURM job submission parameters (partition, time limit, QOS, job name)
- Provide sensible defaults for optional parameters
- Validate file paths and numeric inputs
- Generate three scripts:
  - **config.sh** - Calls `DETECT/create_config.py` to generate `config.json`
  - **run.sh** - Executes the workflow interactively on login node
  - **sbatch_run.sh** - Submits the workflow as a SLURM batch job (recommended)

### 3. Test Configuration (Optional)

Before running the full workflow, test your configuration:

```shell
bash scripts/test_config.sh
```

This will:
- Generate config.json
- Display the configuration
- Run Snakemake in dry-run mode to validate the workflow
- Report any errors without submitting jobs

### 4. Run the Workflow

**Option A: SLURM Batch Submission (Recommended)**

Submit the workflow as a SLURM batch job:

```shell
sbatch sbatch_run.sh
```

Monitor the job:
```shell
squeue -u $USER      # Standard SLURM queue view
snakejobs            # Enhanced view with rule names
```

**Option B: Interactive Execution**

Execute the workflow interactively on the login node:

```shell
./run.sh
```

**Option C: Manual Steps**

Run the steps separately:

```shell
./config.sh              # Generate config.json
snakemake -p --profile profiles/slurm --configfile work/config/config.json -s DETECT/Snakefile
```

## SLURM Configuration

### Job Names in Snakemake 9 vs Snakemake 7

**Important Breaking Change:** In Snakemake 8+, SLURM job names cannot be customized and appear as UUIDs (e.g., `f48e0cfd-54ec-48b9-8857-eee71371cded`) instead of rule names. This is a design decision by the SLURM executor plugin, which uses unique job names for internal job tracking.

In Snakemake 7, rule names were displayed as SLURM job names. This is **no longer possible** in Snakemake 8+.

### Viewing Rule Names in squeue

Snakemake stores the rule name and wildcards in the SLURM job's **comment** field. To view rule names when checking job status, use the comment field (`%k`) in your squeue output format:

```shell
# View jobs with rule names in the comment field
squeue -u acchapm1 -o "%.18i %.9P %.40k %.8u %.2t %.10M"
```

Where:
- `%.18i` = Job ID
- `%.9P` = Partition
- `%.40k` = Comment field (contains rule name and wildcards)
- `%.8u` = User
- `%.2t` = State
- `%.10M` = Time used

For running jobs, you can also use:
```shell
squeue -u acchapm1 -o "%i,%P,%.10j,%.40k"
```

The comment field width can be adjusted (e.g., `%.40k` for 40 characters, `%.60k` for 60 characters) depending on your rule naming conventions.

### Alternative: Use the snakejobs Script

A custom `snakejobs` script is provided that enhances the cluster's `myjobs` command by adding a **RULE** column showing Snakemake rule names from the SLURM comment field.

To install:

```bash
# Run the installation script
bash scripts/install_snakejobs.sh
```

This script will:
1. Copy `scripts/snakejobs` to `$HOME/.local/bin`
2. Verify `$HOME/.local/bin` is in your PATH
3. Add it to your PATH in `~/.bashrc` if needed
4. Update your current environment

After installation, you have two commands available:
```bash
myjobs      # System command - standard job view (no rule names)
snakejobs   # New command - enhanced view with RULE column showing Snakemake rule names
```

The `snakejobs` script provides the same formatted output as `myjobs` with these columns:
- JobID (with array task IDs merged)
- PRIORITY
- PARTITION/QOS (merged)
- NAME
- STATE
- TIME USED
- TIME LIMIT
- Node/Core/GPU (merged)
- **RULE** (Snakemake rule name from comment field)
- REASON

## Documentation

- **README.md** - This file - Quick start guide
- **WORKFLOW_GUIDE.md** - Comprehensive guide to configuration parameters and usage examples
- **CHANGELOG.md** - Recent updates and version history
- **DETECT/README.md** - Original DETECT workflow documentation
- **scripts/create_config.sh** - Interactive configuration script generator

## Files Generated by create_config.sh

When you run `bash scripts/create_config.sh`, it creates:

1. **config.sh** - Shell script that generates `config.json` by calling `DETECT/create_config.py`
2. **run.sh** - Interactive workflow execution script that:
   - Generates config.json by calling config.sh
   - Validates the config was created
   - Runs Snakemake with SLURM executor (on login node)
   - Reports completion and output location
3. **sbatch_run.sh** - SLURM batch submission script that:
   - Includes SBATCH directives for job submission (partition, time, QOS, job name)
   - Generates config.json by calling config.sh
   - Runs Snakemake with SLURM executor (as a batch job)
   - Sends stdout to `o-<jobid>.out` and stderr to `e-<jobid>.err`

## Example Usage

```bash
# Interactive configuration
bash scripts/create_config.sh

# Review generated files
cat config.sh
cat run.sh

# Execute workflow
./run.sh
```

See DETECT/README.md and WORKFLOW_GUIDE.md for more information. 
